...
See [docs/performance.md](docs/performance.md) for what makes generation faster on an 11 GB card (First Block Cache, the new samplers, CFG-Zero*, the launch flags and swap settings), what is not worth trying on Turing-class GPUs, and the dependency roadmap.
...
docker compose up -d # Docker         (any host with the NVIDIA Container Toolkit)
```
