...
# gradio 6 moved the static-file route from /file= to /gradio_api/file=;
# the old prefix 404s, which silently broke the whole ForgeCanvas (JS+CSS
# never loaded -> ForgeCanvas undefined everywhere, eager and lazy).
...
canvas_head += web_js('canvas_lazy_init.js')
...
class ForgeCanvas:
    def __init__(
        ...
        lazy = Context.root_block is None
        if lazy:
            # DEFERRED (lazy tab, e.g. inside gr.render): the demo page already loaded, so a
            # Context.root_block.load(...) handler would never fire for this later-mounted canvas.
            # Emit a hidden marker carrying the JS init args as JSON; canvas_lazy_init.js's
            # MutationObserver instantiates `new ForgeCanvas(...args)` once the container mounts.
            init_args = [self.uuid, no_upload, no_scribbles, contrast_scribbles, height,
                         scribble_color, scribble_color_fixed, scribble_width, scribble_width_fixed,
                         scribble_alpha, scribble_alpha_fixed, scribble_softness, scribble_softness_fixed]
            marker = ('<div class="forge-lazy-canvas" style="display:none" '
                      "data-forge-init='" + json.dumps(init_args) + "'></div>")
