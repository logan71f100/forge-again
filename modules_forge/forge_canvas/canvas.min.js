...
window.ForgeCanvas = class extends window.ForgeCanvas {
    ...
    uploadBase64(value) {
        // remember server-sent (URL) backgrounds so the echo write-back can
        // reuse the marker instead of re-uploading megabytes it just received
        this._bgServerValue = (value && typeof value === 'string' && !value.startsWith('data:')) ? value : null;
        return super.uploadBase64(value);
    }

    on_img_upload() {
        if (!this.img) return super.on_img_upload();
        if (this._bgServerValue) {
            this.background_gradio_bind.set_value(this._bgServerValue);
            return;
        }
        // drawImage() of an <img> that has not finished DECODING silently
        // draws nothing -- no error, no rejection -- so the empty frame then
        // uploads as a fully transparent PNG. Server-side that flattens to
        // the flat background color, so the model is handed a black/white
        // rectangle while the on-screen canvas (which finishes decoding a
        // moment later) shows the real picture. Confirmed from a captured
        // init image: 1920x1080 RGBA, every byte zero, for a run whose mask
        // -- uploaded later, after the decode -- arrived intact. So: wait
        // until the element is actually decoded before drawing, and if it
        // never is, refuse to touch the bind at all. A loud failure beats
        // shipping an empty frame that turns into garbage three steps
        // downstream, where nothing ties it back to this moment.
        const img = document.getElementById('image_' + this.uuid);
        const seq = (this._bgUploadSeq = (this._bgUploadSeq || 0) + 1);
        const t0 = performance.now();
        const ready = () => img && img.complete && img.naturalWidth > 0;
        this._forgeTrackBind(new Promise(resolve => {
            const attempt = (tries) => {
                if (seq !== this._bgUploadSeq) return resolve('superseded');
                if (!ready()) {
                    if (tries > 0) return void ((window.forgeTimer && forgeTimer.setTimeout) || setTimeout)(() => attempt(tries - 1), 100);
                    // Never leave a stale marker for the submit path to trust:
                    // an empty bind makes the run fail loudly server-side
                    // ("no image selected"), which beats generating against
                    // whatever picture the bind referenced ten minutes ago.
                    console.error('[forge-canvas] background image never finished decoding after 10s; bind cleared');
                    this.background_gradio_bind.set_value('');
                    if (window.forgeNotify) forgeNotify.error('The canvas image failed to load — re-add the image and try again.', { id: 'forge-canvas-bg-' + this.uuid });
                    return resolve('failed');
                }
                const waited = performance.now() - t0;
                if (waited > 50) console.warn('[forge-canvas] waited ' + Math.round(waited) + 'ms for background decode before upload (this race used to ship a black frame)');
                const tc = this.temp_canvas;
                const ctx = tc.getContext('2d');
                tc.width = this.orgWidth; tc.height = this.orgHeight;
                ctx.drawImage(img, 0, 0, this.orgWidth, this.orgHeight);
                this._forgeUploadCanvasPNG(tc)
                    .then(p => { if (seq === this._bgUploadSeq) this.background_gradio_bind.set_value('forge-file:' + p); resolve('ok'); })
                    .catch(e => { console.warn('[forge-canvas] file upload failed, falling back to base64:', e); try { super.on_img_upload(); } finally { resolve('fallback'); } });
            };
            attempt(100); // up to ~10s of 100ms polls
        }));
    }

    on_drawing_canvas_upload() {
        if (!this.img) return super.on_drawing_canvas_upload();
        const dc = document.getElementById('drawingCanvas_' + this.uuid);
        this._forgeTrackBind(
            this._forgeUploadCanvasPNG(dc)
                .then(p => this.foreground_gradio_bind.set_value('forge-file:' + p))
                .catch(e => { console.warn('[forge-canvas] mask upload failed, falling back to base64:', e); super.on_drawing_canvas_upload(); })
        );
    }

    // Every bind write is registered here while in flight, so the generate
    // gate below can hold a submit until the textareas actually reflect the
    // canvas. Entries always settle: the decode poll self-times-out and
    // fetch failures land in the catch, so nothing can hold the gate forever.
    _forgeTrackBind(work) {
        const pending = (window._forgeCanvasPending = window._forgeCanvasPending || new Set());
        const entry = { promise: Promise.resolve(work).catch(() => {}).finally(() => pending.delete(entry)) };
        pending.add(entry);
...
// --- generate gate ---------------------------------------------------------
// The transport mirrors canvas state into bind textareas asynchronously, and
// gradio reads those textareas the moment Generate is clicked. Any click that
// lands inside an upload window submits a STALE bind -- the previous image,
// or the empty frame the decode race used to ship. The decode-wait above
// fixes the known cause; this gate closes the whole class: if any canvas
// upload is still in flight, hold the click, let the uploads settle (each one
// self-times-out, so this cannot hang), then replay it. Capture phase so it
// beats gradio's own listener, same trick the pause/resume interceptor uses.
if (!window._forgeCanvasGateInstalled) {
    window._forgeCanvasGateInstalled = true;
    document.addEventListener('click', function (ev) {
        const btn = ev.target && ev.target.closest ? ev.target.closest('button[id$="_generate"]') : null;
        const pending = window._forgeCanvasPending;
        if (!btn || !pending || pending.size === 0) return;
        ev.preventDefault();
        ev.stopImmediatePropagation();
        const waiting = [...pending].map(e => e.promise);
        console.warn('[forge-canvas] generate held for ' + waiting.length + ' in-flight canvas upload(s)');
        if (window.forgeNotify) forgeNotify.info('Waiting for the canvas image to finish uploading…', { id: 'forge-canvas-gate', timeout: 4000 });
        Promise.allSettled(waiting).then(() => {
            // one macrotask of cushion so the binds' input events reach gradio
            ((window.forgeTimer && forgeTimer.setTimeout) || setTimeout)(() => btn.click(), 30);
        });
    }, true);
}
