...
// handleDraw has an erase branch (globalCompositeOperation 'destination-out',
// taken when scribbleAlpha == 0) -- but the inpaint MASK canvas draws through
// the contrast_scribbles (checkerboard) path, which returns before that check,
// so eraser mode wraps handleDraw and forces the erase branch. The click
// listener is DELEGATED on document because gradio 6 can re-render the HTML
// block (replacing the button and orphaning a direct listener); delegation
// keeps the toggle alive across remounts, and _eraser_sync() re-applies the
// visual state to whatever button element currently exists.
window.ForgeCanvas = class extends window.ForgeCanvas {
    constructor(...args) {
        ...
        try {
            ...
            if (this.no_scribbles) return; // display-only canvas: keep it hidden
            this._eraser_sync();
            document.addEventListener('click', (ev) => {
                const b = ev.target && ev.target.closest ? ev.target.closest('#eraserButton_' + uuid) : null;
                if (!b) return;
                this.eraser_mode = !this.eraser_mode;
                this._eraser_sync();
            });
        } catch (e) {
    ...
    _eraser_sync() {
        const btn = document.getElementById('eraserButton_' + this.uuid);
        if (!btn) return;
        btn.style.display = this.no_scribbles ? 'none' : '';
        btn.classList.toggle('forge-btn-active', !!this.eraser_mode);
        btn.dataset.eraser = String(!!this.eraser_mode);
    }
    handleDraw(e) {
        this._eraser_sync(); // self-heal the button state after any remount
        if (!this.eraser_mode) return super.handleDraw(e);
