...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False, head=canvas_head) as txt2img_interface:
        ...
        with gr.Tab("Generation", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(equal_height=False):
            ...
            for component in hr_resolution_preview_inputs:
                ...
                event(
                    None,
                    _js="onCalcResolutionHires", queue=False,
                    inputs=hr_resolution_preview_inputs,
                    outputs=[],
                    show_progress=False,
                )
            ...
            res_switch_btn.click(lambda w, h: (h, w), inputs=[width, height], outputs=[width, height], show_progress=False, queue=False)
    ...
    # LAZY IMG2IMG: img2img is built INLINE here (unlike Settings, a separate Blocks). To defer it
    # with the proven gr.render/build-once-gate template, its whole body is wrapped in this nested
    # builder invoked lazily the first time the img2img tab is selected. The body below keeps its
    # original 8-space indentation UNCHANGED -- only the former `with gr.Blocks(...) as
    # img2img_interface:` line becomes this `def`. The real img2img_interface Blocks (holding just
    # the gate + gr.render) is created after this function; the builder closes over it and runs
    # long after page load, so every reference resolves fine.
    def _build_img2img_body():
        # This deferred build runs after the eager section below has reset scripts_current to None,
        # so re-point it at the img2img ScriptRunner while the body constructs its script UIs.
        scripts.scripts_current = scripts.scripts_img2img
                    ...
                    if category == "image":
                        with gr.Tabs(elem_id="mode_img2img"):
                            ...
                            with gr.TabItem('Batch', id='batch', elem_id="img2img_batch_tab") as tab_batch:
                                ...
                                tab_batch_upload.select(fn=lambda: "upload", inputs=[], outputs=[img2img_batch_source_type], queue=False, show_progress=False)
                                tab_batch_from_dir.select(fn=lambda: "from dir", inputs=[], outputs=[img2img_batch_source_type], queue=False, show_progress=False)
                            ...
                            for i, tab in enumerate(img2img_tabs):
                                tab.select(fn=lambda tabnum=i: tabnum, inputs=[], outputs=[img2img_selected_tab], queue=False, show_progress=False)
                                ...
                                with gr.Tabs(elem_id="img2img_tabs_resize"):
                                    ...
                                    with gr.Tab(label="Resize by", id="by", elem_id="img2img_tab_resize_by") as tab_scale_by:
                                        ...
                                        with FormRow():
                                            ...
                                            # clicked from JS (ui.js) -- must be mounted; gradio 6 unmounts visible=False
                                            button_update_resize_to = gr.Button(visible=True, elem_classes=['webui-hidden-mounted'], elem_id="img2img_update_resize_to")
                                    ...
                                    for i in img_sources:
                                        i.change(fn=updateWH, inputs=[i, width, height], outputs=[width, height], show_progress='hidden', queue=False)
                                        i.change(**on_change_args)

                            tab_scale_to.select(fn=lambda: 0, inputs=[], outputs=[selected_scale_tab], queue=False, show_progress=False)
                            tab_scale_by.select(fn=lambda: 1, inputs=[], outputs=[selected_scale_tab], queue=False, show_progress=False)
        ...
        # LAZY IMG2IMG: wire the "Send to img2img"/"Send to inpaint" paste buttons now that the
        # img2img body (and thus paste_fields["img2img"]/["inpaint"]) exists. These bindings --
        # registered by the txt2img/PNG Info/Extras output panels and by img2img's own toprow.paste
        # -- were skipped at page load because their destination didn't exist yet. The call is
        # idempotent, so already-wired bindings are untouched.
        parameters_copypaste.connect_paste_params_buttons()

        # LAZY IMG2IMG: this deferred build runs after page load, so the img2img extra-networks
        # (Lora/etc.) interface.load registered inside ui_extra_networks.create_ui above never
        # fires. Populate the card HTML directly so the Lora pages aren't blank until Refresh.
        try:
            extra_networks_ui_img2img.pages_contents = [
                pg.create_html(extra_networks_ui_img2img.tabname)
                for pg in extra_networks_ui_img2img.stored_extra_pages
            ]
            for _pg_elem, _pg_html in zip(extra_networks_ui_img2img.pages, extra_networks_ui_img2img.pages_contents):
                _pg_elem.value = _pg_html
    ...
    with gr.Blocks(analytics_enabled=False, head=canvas_head) as img2img_interface:
        # LAZY IMG2IMG: only the build-once gate lives eagerly. The gr.render fires exactly once on
        # the first False->True gate transition (flipped by the img2img TabItem.select handler in
        # the Tabs loop below) and constructs the whole body via _build_img2img_body().
        img2img_built_gate = gr.State(False)
    ...
    with gr.Blocks(analytics_enabled=False) as extras_interface:
        # LAZY EXTRAS: build the Extras (postprocessing) tab body only when the tab is first
        # selected. The gate is flipped True by the Extras TabItem.select handler in the Tabs
        # loop below; gr.State.change fires only on a real False->True transition so the render
        # runs exactly once.
        extras_built_gate = gr.State(False)
        ...
        def _render_extras_body():
            ...
            # gallery's own outgoing "Send to img2img/inpaint" buttons. The call is idempotent,
            # so already-wired bindings are left untouched.
            parameters_copypaste.connect_paste_params_buttons()

    extras_interface.extras_built_gate = extras_built_gate

    with gr.Blocks(analytics_enabled=False) as pnginfo_interface:
        # LAZY PNG INFO: build the PNG Info tab body only when the tab is first selected. The
        # gate is flipped True by the PNG Info TabItem.select handler in the Tabs loop below.
        pnginfo_built_gate = gr.State(False)
        ...
        def _render_pnginfo_body():
            with ResizeHandleRow(equal_height=False):
                ...
                with gr.Column(variant='panel'):
                    ...
                    # PNG Info is the SOURCE side of these bindings (its image/generation_info feed
                    # txt2img/img2img/inpaint/extras). Register them; the connect call below wires
                    # them to whichever destinations are already built. A destination that is itself
                    # lazy and not yet built is skipped and gets wired when it builds (its own
                    # connect_paste_params_buttons() call scans all registered bindings incl. these).
            ...
            # Idempotent: wires PNG Info's four Send-to buttons to their already-built destinations.
            parameters_copypaste.connect_paste_params_buttons()

    pnginfo_interface.pnginfo_built_gate = pnginfo_built_gate
    ...
    # LAZY EXTENSIONS: build the Extensions tab body (ui_extensions.create_ui, which also kicks
    # off git-metadata preloading and config-state listing) only when the tab is first selected.
    # create_ui() builds its own gr.Blocks and is fully self-contained (nothing outside references
    # its components), so calling it inside the gr.render nests it under the deferred column.
    with gr.Blocks(analytics_enabled=False) as extensions_interface:
        extensions_built_gate = gr.State(False)
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion", head=canvas_head) as demo:
        ...
        # #settings_json must be in the DOM at all times: ui.js reads it to populate
        # window.opts (progress bar, live previews, hints, ...). gradio 6 only mounts the
        # active tab's content, so it cannot live inside the Settings tab -- render it here
        # at the always-mounted top level (CSS-hidden via .webui-hidden-mounted).
        ...
        with gr.Tabs(elem_id="tabs") as tabs:
            ...
            settings_tabitem = None
            extras_tabitem = None
            pnginfo_tabitem = None
            modelmerger_tabitem = None
            extensions_tabitem = None
            img2img_tabitem = None
            for interface, label, ifid in sorted_interfaces:
                if label in shared.opts.hidden_tabs:
                    continue
                with gr.TabItem(label, id=ifid, elem_id=f"tab_{ifid}") as tabitem:
                    interface.render()

                if ifid == "settings":
                    settings_tabitem = tabitem

                if ifid == "extras":
                    extras_tabitem = tabitem

                if ifid == "pnginfo":
                    pnginfo_tabitem = tabitem

                if ifid == "modelmerger":
                    modelmerger_tabitem = tabitem

                if ifid == "extensions":
                    extensions_tabitem = tabitem

                if ifid == "img2img":
                    img2img_tabitem = tabitem
            ...
            # LAZY SETTINGS: flip the build-once gate the first time the Settings tab is selected.
            # This drives the gr.render() in ui_settings that constructs the heavy Settings body.
            # loadsave.setup_ui() (the "Defaults" tab wiring) is now performed inside that deferred
            # build, so it is intentionally no longer called eagerly here.
            # NOTE: all these gate flips must stay QUEUED (no queue=False): gradio 6 routes
            # unqueued events over bare /run/predict, whose responses cannot carry the
            # render_config that delivers the gr.render() tab body -- the tab would select
            # but stay empty forever.
            if settings_tabitem is not None:
                settings_tabitem.select(fn=lambda: True, outputs=[settings.settings_built_gate], show_progress=False)

            # LAZY EXTRAS: flip the build-once gate the first time the Extras tab is selected,
            # driving the gr.render() that builds the postprocessing body (see extras_interface).
            if extras_tabitem is not None:
                extras_tabitem.select(fn=lambda: True, outputs=[extras_interface.extras_built_gate], show_progress=False)

            # LAZY PNG INFO: flip the build-once gate the first time the PNG Info tab is selected.
            if pnginfo_tabitem is not None:
                pnginfo_tabitem.select(fn=lambda: True, outputs=[pnginfo_interface.pnginfo_built_gate], show_progress=False)

            # LAZY CHECKPOINT MERGER: flip the gate the first time the Checkpoint Merger tab is
            # selected, driving the gr.render() that builds the merger body + wires its events.
            if modelmerger_tabitem is not None:
                modelmerger_tabitem.select(fn=lambda: True, outputs=[modelmerger_ui.modelmerger_built_gate], show_progress=False)

            # LAZY EXTENSIONS: flip the gate the first time the Extensions tab is selected.
            if extensions_tabitem is not None:
                extensions_tabitem.select(fn=lambda: True, outputs=[extensions_interface.extensions_built_gate], show_progress=False)

            # LAZY IMG2IMG: flip the gate the first time the img2img tab is selected, driving the
            # gr.render() that builds the img2img body + wires its events. "Send to img2img/inpaint"
            # buttons switch to this tab (switch_to_img2img/inpaint js), which selects it and thus
            # flips this gate, so the destination is built before the paste executes.
            if img2img_tabitem is not None:
                img2img_tabitem.select(fn=lambda: True, outputs=[img2img_interface.img2img_built_gate])

        # gradio 6 REMOUNTS a container when it receives a visibility update, which
        ...
        # LAZY IMG2IMG: the image_cfg_scale (instruct-pix2pix) visibility wiring that used to live
        # here referenced the img2img body's image_cfg_scale slider, which is now built lazily. It
        # has been moved into _build_img2img_body() so it binds to the freshly-built component.
