...
def connect_paste_params_buttons(only_tabname=None, bindings=None):
    """Wire the registered "Send to <tab>" paste buttons to their destination components.

    A paste binding can only be wired once BOTH ends exist: its source component (the button and
    the image/prompt it copies FROM) and its destination tab's paste fields (what it copies TO).
    With lazily-built tabs either end may appear late, so this function is designed to be called
    repeatedly and idempotently:

      * The eager page-load call wires every binding whose destination already exists.
      * Every lazily-built tab calls it (no args) at the end of its deferred build. That single
        call wires all of the tab's own buttons (incoming "Send to <me>" AND its output gallery's
        outgoing "Send to <other>") plus any earlier-skipped cross-link whose other end is now
        present -- regardless of the order tabs are opened.

    Bindings whose destination tab is not yet built (paste fields absent) are skipped and picked
    up by a later call. `_connected_bindings` guarantees each binding is wired at most once.
