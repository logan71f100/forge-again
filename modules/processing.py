...
class StableDiffusionProcessing:
    ...
    def inpainting_image_conditioning(self, source_image, latent_image, image_mask=None, round_image_mask=True):
        ...
        if getattr(self.sd_model, 'is_flux_fill', False):
            # Flux Fill conditioning: c_concat = [masked_image_latent(16ch), packed_mask(64ch)] = 80ch.
            # mask==1 marks the region to regenerate; masked image has that region zeroed (context for the model).
            masked_image = source_image * (1.0 - conditioning_mask)
            masked_latent = self.sd_model.get_first_stage_encoding(self.sd_model.encode_first_stage(masked_image))
            lat_h, lat_w = masked_latent.shape[-2], masked_latent.shape[-1]
            # pack the pixel-res mask into 64 channels at latent res (8x8 pixel-unshuffle, matches Flux Fill training).
            m = torch.nn.functional.interpolate(conditioning_mask, size=(lat_h * 8, lat_w * 8))
            packed_mask = torch.nn.functional.pixel_unshuffle(m, 8).to(masked_latent)
            packed_mask = packed_mask.expand(masked_latent.shape[0], -1, -1, -1)
            return torch.cat([masked_latent, packed_mask], dim=1)
    ...
    def img2img_image_conditioning(self, source_image, latent_image, image_mask=None, round_image_mask=True):
        ...
        if self.sd_model.is_inpaint or getattr(self.sd_model, 'is_flux_fill', False):
