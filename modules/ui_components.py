    ...
    def __init__(self, value=None, setup=False, **kwargs):
        ...
        kwargs_checkbox = {
            **kwargs,
            ...
            # gradio 6 does not mount visible=False components in the DOM, but
            # inputAccordion.js must find this checkbox to mirror the accordion's
            # open state into it (Hires fix / Refiner enable toggles). Mount it
            # CSS-hidden instead (same pattern as the other JS-bridge elements).
            "visible": True,
            "elem_classes": (list(kwargs.get('elem_classes') or []) + ['webui-hidden-mounted']),
        }
