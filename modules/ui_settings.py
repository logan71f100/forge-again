...
class UiSettings:
    ...
    def create_ui(self, loadsave, dummy_component):
        ...
        # LAZY SETTINGS: the ~545-component Settings tab body is the heaviest safely-deferrable
        # part of the page. Instead of building every setting control up front, we build only a
        # tiny eager scaffold here, and construct the full body via gr.render() the first time
        # the Settings tab is selected (see _build_settings_body + the gate flip in modules/ui.py).
        #
        # The quicksettings *metadata* (self.quicksettings_list) must still be computed eagerly,
        # because add_quicksettings() builds the top quicksettings bar at page-load time, long
        # before the Settings tab is ever opened. Only the per-setting component *construction*
        # is deferred.
        ...
        with gr.Blocks(analytics_enabled=False) as settings_interface:
            ...
            # Kept EAGER: text_settings is referenced from modules/ui.py (text_settings.change)
            # and by the quicksettings handlers (run_settings_single) which are wired at page
            # build time, so it must exist before the Settings tab is opened.
            #
            # gradio 6 only mounts the ACTIVE tab's DOM, and does not mount visible=False
            # components at all (gradio 4 mounted everything, hidden). ui.js reads
            # #settings_json's textarea to populate window.opts, which nearly all frontend JS
            # (progress bar, live previews, hints, zoom) depends on -- so this is created
            # render=False here and rendered CSS-hidden at the demo's always-mounted top level
            # in modules/ui.py, next to the quicksettings row.
            self.text_settings = gr.Textbox(elem_id="settings_json", value=lambda: opts.dumpjson(), visible=True, elem_classes=['webui-hidden-mounted'], render=False)
            ...
            # Build-once gate. modules/ui.py flips this State to True the first time the Settings
            # tab is selected. gr.State.change only fires on a real value transition (False->True),
            # so the render below runs exactly once even though the tab can be selected repeatedly.
            self.settings_built_gate = gr.State(False)
    ...
    def _build_settings_body(self):
        """Constructs the heavy Settings tab body. Runs lazily inside a gr.render(), triggered
        the first time the Settings tab is selected. Any event wiring that references the
        deferred components (Apply button, per-setting handlers, loadsave Defaults) is registered
        HERE so it binds to the freshly-built components."""
        loadsave = self._loadsave

        # Reset the ordered component list so a (guarded) re-render stays consistent. We do NOT
        # clear component_dict, because add_quicksettings() has already populated it with the
        # eager quicksettings components and shared.settings_components points at the same dict.
        self.components = []
        ...
        with gr.Tabs(elem_id="settings"):
            for i, (k, item) in enumerate(opts.data_labels.items()):
                ...
                if k in self.quicksettings_names and not shared.cmd_opts.freeze_settings:
                    self.components.append(self.dummy_component)
        ...
        # loadsave "Defaults" tab wiring. Moved out of modules/ui.py (which used to call
        # loadsave.setup_ui() eagerly) because the ui_defaults_view/apply buttons only exist
        # after this deferred build. All other tabs are registered via add_block() at page-load
        # time, so component_mapping is fully populated by the time the Settings tab is opened.
        loadsave.setup_ui()

        # "Apply settings" wiring. Needs the full ordered self.components list, which only exists
        # after the loop above, so it is registered here rather than in add_functionality().
        self.submit.click(
    ...
    def add_functionality(self, demo):
        # NOTE: self.submit.click ("Apply settings") is wired inside _build_settings_body()
        # instead of here, because it needs the full self.components list which is built lazily
        # when the Settings tab is first opened.
        ...
        # clicked from JS (selectCheckpoint in ui.js) -- must be mounted; gradio 6 unmounts visible=False
        button_set_checkpoint = gr.Button('Change checkpoint', elem_id='change_checkpoint', visible=True, elem_classes=['webui-hidden-mounted'])
