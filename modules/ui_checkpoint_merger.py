...
class UiCheckpointMerger:
    def __init__(self):
        # LAZY MERGER: build only a hidden gr.State build-once gate up front; construct the whole
        # Checkpoint Merger body (and wire its events) lazily via gr.render the first time the tab
        # is selected. modules/ui.py flips the gate on the Checkpoint Merger TabItem.select.
        #
        # setup_ui() is called at page-build time (before the tab is ever opened) with the eager
        # dummy_component and the sd_model_checkpoint quicksettings dropdown; we only stash those
        # args and perform the actual event wiring inside the deferred build, once the merger
        # components exist.
        self._setup_args = None

        with gr.Blocks(analytics_enabled=False) as modelmerger_interface:
            self.modelmerger_built_gate = gr.State(False)
    ...
    def setup_ui(self, dummy_component, sd_model_checkpoint_component):
        # LAZY MERGER: called at page-build time, before the tab body exists. Only stash the eager
        # args; the actual event wiring runs in _wire_events() from inside the deferred build.
        self._setup_args = dict(dummy_component=dummy_component, sd_model_checkpoint_component=sd_model_checkpoint_component)

    def _wire_events(self, dummy_component, sd_model_checkpoint_component):
        ...
        self.modelmerger_merge.click(
            fn=call_queue.wrap_gradio_gpu_call(modelmerger, extra_outputs=[gr.update() for _ in range(4)]),
            _js='modelmerger',
            inputs=[
                dummy_component,
