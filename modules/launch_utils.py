...
def run_pip(command, desc=None, live=default_command_live):
    ...
    # --no-build-isolation: legacy sdists (openai CLIP, etc.) have setup.py that
    # imports pkg_resources, which pip's ISOLATED build env removed (newest
    # setuptools). Building against the venv's pinned setuptools<81 (which still
    # ships pkg_resources) fixes "ModuleNotFoundError: No module named
    # 'pkg_resources'" on first-run installs under the modern gradio-6 stack.
    isolation = ' --no-build-isolation' if command.strip().startswith('install') else ''
    return run(f'"{python}" -m pip {command} --prefer-binary{isolation}{index_url_line}', desc=f"Installing {desc}", errdesc=f"Couldn't install {desc}", live=live)
...
def prepare_node():
    """Ensure a portable Node runtime exists for gradio 6 server-side rendering.

    SSR (the big page-load speedup in gradio 6) shells out to a Node process.
    Rather than require a system-wide/admin Node install, we bootstrap a portable
    Node into ./node on first run — exactly like the venv holds python deps.
    SSR is OPT-IN (SD_WEBUI_SSR=1) because it's a net negative for localhost —
    only useful for LAN/remote access. So we only fetch Node when it's requested.
    Skipped if Node is already present or on non-Windows (system node on PATH).
    Failure is non-fatal: SSR stays off and the UI uses client-side render.
    """
    if os.getenv("SD_WEBUI_SSR") != "1":
        return
    node_dir = os.path.join(script_path, "node")
    node_exe = os.path.join(node_dir, "node.exe" if os.name == "nt" else os.path.join("bin", "node"))
    if os.path.isfile(node_exe):
        return
    if os.name != "nt":
        # on linux/mac a system node on PATH is the norm; don't fetch a windows zip
        return
    try:
        ...
        print("Bootstrapping portable Node for gradio SSR (first run only)...")
        idx = requests.get("https://nodejs.org/dist/index.json", timeout=30).json()
        ver = next(r["version"] for r in idx if r["lts"])
        url = f"https://nodejs.org/dist/{ver}/node-{ver}-win-x64.zip"
        zip_path = os.path.join(script_path, "node_dl.zip")
        with requests.get(url, stream=True, timeout=600) as r:
            r.raise_for_status()
            with open(zip_path, "wb") as f:
                for chunk in r.iter_content(chunk_size=1 << 20):
                    f.write(chunk)
        tmp = os.path.join(script_path, "node_tmp")
        with zipfile.ZipFile(zip_path) as z:
            z.extractall(tmp)
        inner = os.path.join(tmp, f"node-{ver}-win-x64")
        os.rename(inner, node_dir)
        os.rmdir(tmp)
        os.remove(zip_path)
        print(f"Portable Node {ver} installed to ./node")
    except Exception as e:
        print(f"[node] portable Node bootstrap failed ({e}); SSR will be disabled, "
              f"UI falls back to client-side render. Set SD_WEBUI_NO_SSR=1 to silence.")


def is_rocm_requested():
    """True when this launch should use the AMD ROCm torch build.

    Either the --rocm flag or FORGE_GPU=rocm, which start.sh exports after its
    GPU auto-detection. getattr keeps this safe if an older cmd_args (or an
    embedding host) has no --rocm defined.
    """
    if getattr(args, 'rocm', False):
        return True
    return os.environ.get('FORGE_GPU') == 'rocm'
...
def prepare_environment():
    ...
    # AMD ROCm: same torch/torchvision VERSIONS as the CUDA path, from the ROCm
    # index. Verified to exist for cp312: torch 2.13.0+rocm7.1 and torchvision
    # 0.28.0+rocm7.1. Keep these in lockstep with requirements_versions.txt --
    # a mismatched pair (e.g. torchvision 0.18, which pairs with torch 2.3) does
    # not resolve at all. Override the ROCm level with TORCH_ROCM_INDEX.
    elif is_rocm_requested():
        torch_index_url = os.environ.get('TORCH_ROCM_INDEX', "https://download.pytorch.org/whl/rocm7.1")
        torch_command = os.environ.get('TORCH_COMMAND', f"pip install torch==2.13.0 torchvision==0.28.0 --index-url {torch_index_url}")
    ...
    prepare_node()
    startup_timer.record("prepare node (SSR)")
