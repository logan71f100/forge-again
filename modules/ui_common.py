        ...
        with gr.Column(variant='panel', elem_id=f"{tabname}_results_panel"):
            ...
            if tabname != "extras":
                ...
                with gr.Group():
                    ...
                    # JS bridge elements: generationParams.js clicks the hidden button and reads
                    # the textbox from the DOM. gradio 6 does not mount visible=False components
                    # (gradio 4 mounted them hidden), so mount visibly + hide via CSS.
                    res.generation_info = gr.Textbox(visible=True, elem_classes=['webui-hidden-mounted'], elem_id=f'generation_info_{tabname}')
                    if tabname == 'txt2img' or tabname == 'img2img':
                        generation_info_button = gr.Button(visible=True, elem_classes=['webui-hidden-mounted'], elem_id=f"{tabname}_generation_info_button")
