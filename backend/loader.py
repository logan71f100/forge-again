...
try:
    # transformers >= 5 moved no_init_weights out of modeling_utils
    ...
    if lib_name in ['transformers', 'diffusers']:
        ...
        if component_name.startswith('text_encoder') and cls_name in ['CLIPTextModel', 'CLIPTextModelWithProjection']:
            ...
            if not hasattr(model.transformer, 'text_model'):
                ...
                # are direct children). Checkpoint keys still carry the old `text_model.` infix,
                # so drop it to match the flattened parameter names.
                state_dict = {k.replace('transformer.text_model.', 'transformer.', 1): v for k, v in state_dict.items()}
