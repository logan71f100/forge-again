...
class PredictionFlux(AbstractPrediction):
    ...
    def apply_mu_transform(self, seq_len=4096, base_seq_len=256, max_seq_len=4096, base_shift=0.5, max_shift=1.15, mu=None):
        ...
        # diffusers 0.39: time_shift became an instance method that reads
        # self.config.time_shift_type, so the old unbound call
        # FlowMatchEulerDiscreteScheduler.time_shift(None, mu, 1.0, sigmas) crashes.
        # Inline the exponential time shift (what that call always computed):
