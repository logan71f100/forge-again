...
def _clip_text_model(transformer):
    # transformers >= 5 flattened CLIPTextModel: embeddings/encoder/final_layer_norm
    # are direct children. transformers < 5 nested them under `.text_model`.
...
class ClassicTextProcessingEngine:
    def __init__(
        ...
        model_embeddings = _clip_text_model(text_encoder.transformer).embeddings
    ...
    def encode_with_transformers(self, tokens):
        ...
        text_model = _clip_text_model(self.text_encoder.transformer)
        text_model.embeddings.position_ids = text_model.embeddings.position_ids.to(device=target_device)
        text_model.embeddings.position_embedding = text_model.embeddings.position_embedding.to(dtype=torch.float32)
        text_model.embeddings.token_embedding = text_model.embeddings.token_embedding.to(dtype=torch.float32)
        ...
        if self.final_layer_norm:
            z = _clip_text_model(self.text_encoder.transformer).final_layer_norm(z)
