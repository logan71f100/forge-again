...
class Flux(ForgeDiffusionEngine):
    ...
    def __init__(self, estimated_config, huggingface_components):
        ...
        # Flux Fill models build img_in for 384 features (96 channels x 2x2 patchify) instead of 64.
        # Flag them so the inpaint conditioning builder produces Flux-style 80-ch c_concat
        # (masked-image latent 16ch + 8x8-packed mask 64ch); routing then makes 16+80=96 -> 384.
        _transformer = huggingface_components['transformer']
        # The actual loaded img_in weight is ground truth (3072x384 for Fill, 3072x64 for base);
        # the module's in_channels config can be mis-detected (base) even when Fill weights load.
        _img_in = getattr(_transformer, 'img_in', None)
        _in_feat = None
        _w = getattr(_img_in, 'weight', None)
        if _w is not None and getattr(_w, 'ndim', 0) >= 2:
            _in_feat = int(_w.shape[-1])
        if _in_feat is None:
            _in_feat = getattr(_img_in, 'in_features', None) or getattr(_transformer, 'in_channels', None)
        self.is_flux_fill = bool(_in_feat == 384)
