...
(function () {
    ...
    function systemPrompt(controls) {
        ...
        return [
            ...
            '- {"tool":"generate"} — run the generation. This temporarily parks you to free VRAM and brings you back with the result attached. Once the user has asked for image work, use it proactively — settings changes accomplish nothing until a generation runs. It is NEVER a valid response to a question.',
            ...
            'QUESTION vs COMMAND — decide this FIRST, every single turn, before anything else: if the user\'s latest message is a question or discussion ("what do you think", "why did", "how should", "describe", "what prompt would you use"), your ENTIRE reply is plain text — ZERO tool calls. No set, no generate, nothing. SUGGEST instead of doing: write the prompt you would use as text and end with "want me to apply it?". Tools are for turns where the user explicitly asked for action ("do it", "apply that", "generate", "change the prompt", "fix the hands"). If you are not sure which kind of turn this is, it is a question. An unwanted answer costs nothing; an unwanted generation costs the user a run and overwrites their settings.',
            ...
            'ITERATION LOOP — applies ONLY after the user has requested image work (see QUESTION vs COMMAND above; a question never starts this loop): work autonomously — make your change(s) AND include {"tool":"generate"} in the SAME reply, evaluate, record the verdict, adjust, generate again, indefinitely (see CHASE MODE). The user stops you with ⏹. Within the loop, never end a turn having changed settings without generating.',
