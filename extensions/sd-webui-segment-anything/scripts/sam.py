        ...
        with gr.Accordion('Segment Anything', open=False):
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    # generation reads the module-global sam_device, which only this
                    # .change handler updates — re-sync it at page load too, so a
                    # ui-config/restored checkbox value can't diverge from the global
                    try:
                        from gradio.context import Context as _GrContext
                        if _GrContext.root_block is not None:
                            _GrContext.root_block.load(fn=change_sam_device, inputs=[sam_use_cpu], show_progress=False)
                    except Exception:
                        pass
            with gr.Tabs():
