...
def applyHiresFix(
    ...
    for image in gallery:
        ...
        _d = image[0] if isinstance(image, (tuple, list)) else image
        _path = (_d.get("path") or _d.get("name") or "") if isinstance(_d, dict) else str(_d)
        fake_image.already_saved_as = str(_path).rsplit("?", 1)[0]
