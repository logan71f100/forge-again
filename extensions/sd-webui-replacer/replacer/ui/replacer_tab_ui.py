...
def _makeExampleChips(examples, target_elem_id, elem_id):
    """Clickable example chips that REPLACE the target prompt field's content.

    Replaces gr.Examples here: gradio 6's Dataset renders text samples as blank
    buttons (empty spans holding ~180px of space). One full-width button per
    example so the whole text is readable (buttons wrap via CSS)."""
    with gr.Column(elem_id=elem_id, elem_classes=["replacer-example-chips"], min_width=0):
        for ex in examples:
            js = ("() => {var t=document.querySelector('#" + target_elem_id + " textarea');"
                  " if(t){t.value=" + _json.dumps(ex) + ";"
                  " t.dispatchEvent(new Event('input',{bubbles:true}));}}")
            gr.Button(ex, size="sm", min_width=0).click(fn=None, js=js)
...
def getTabUI(comp: AttrDict, isDedicatedPage: bool):
    with gr.Blocks(analytics_enabled=False) as replacerTabUI:
        ...
        comp.dummy_component = gr.Textbox(visible=False)  # was gr.Label — gradio4 LabelData crash on Run (it's just a string task_id carrier)
            ...
            with gr.Column(scale=16):
                ...
                with gr.Row():
                    ...
                    if getDetectionPromptExamplesNumber() > 0:
                        _makeExampleChips(getDetectionPromptExamples(),
                            "replacer_detectionPrompt", "replacer_detectionPrompt_examples")
                ...
                with gr.Row():
                    ...
                    if getPositivePromptExamplesNumber() > 0:
                        _makeExampleChips(getPositivePromptExamples(),
                            "replacer_positivePrompt", "replacer_positivePrompt_examples")
                ...
                with gr.Row():
                    ...
                    if getNegativePromptExamplesNumber() > 0:
                        _makeExampleChips(getNegativePromptExamples(),
                            "replacer_negativePrompt", "replacer_negativePrompt_examples")
