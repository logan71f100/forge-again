...
def inpaint(
    ...
    try:
        ...
        _g = _json.load(open(_os.path.join(_webui_root, "mode_profile.json"), encoding="utf-8")).get("REPLACER_FLUX_GUIDANCE")
    except Exception:
        _g = None
    p.distilled_cfg_scale = float(_g if _g is not None else _os.environ.get('REPLACER_FLUX_GUIDANCE', '3.5'))
