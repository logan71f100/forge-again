...
function setSubmitButtonsVisibility(tabname, showInterrupt, showSkip, showInterrupting) {
    // null-safe: not every tab has all three buttons (e.g. Replacer)
    var set = function(id, show) {
        var el = gradioApp().getElementById(id);
        if (el) el.style.display = show ? "block" : "none";
    };
    set(tabname + '_interrupt', showInterrupt);
    set(tabname + '_skip', showSkip);
    set(tabname + '_interrupting', showInterrupting);
}
...
function updateInput(target) {
    ...
    // gradio 6's Svelte binds checkboxes/radios to 'change', not 'input' —
    // without this the backend never sees programmatic toggle updates
    if (target.type === 'checkbox' || target.type === 'radio') {
        let c = new Event("change", {bubbles: true});
        Object.defineProperty(c, "target", {value: target});
