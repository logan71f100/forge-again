    ...
    onUiUpdate(() => {
        ...
        class ControlNetUnitTab {
            constructor(tab, accordion) {
                ...
                // checkbox (an inner '.cnet-unit-enabled' checkbox only exists on
                // legacy builds)
                this.enabledCheckbox = tab.querySelector('.cnet-unit-enabled input') ||
                    gradioApp().querySelector('#' + tab.id + '-checkbox input');
                ...
                // (unlinking open-state from the checkbox is handled by the
                // 'input-accordion-unlinked' class in inputAccordion.js now —
                // the old double-click trick would fire spurious enable events)
            ...
            sync_enabled_checkbox() {
                // bridge mode: the header checkbox IS the enable component and
                // inputAccordion.js already keeps visual and bridge in sync
                if (!this.tab.querySelector('.cnet-unit-enabled input')) return;
            ...
            updateActiveUnitCount() {
                ...
                const checkboxes = [...this.accordion.querySelectorAll('.input-accordion')]
                    .map(t => t.querySelector('.cnet-unit-enabled input') ||
                              gradioApp().querySelector('#' + t.id + '-checkbox input'))
                    .filter(Boolean);
        ...
        gradioApp().querySelectorAll('#controlnet').forEach(accordion => {
            // gradio 6 only mounts a closed accordion's contents when it opens, so
            // the unit tabs appear long after page load. Wire each tab when it
            // first shows up (per-tab marker) instead of once per accordion —
            // otherwise the header-toggle <-> Enable sync never attaches and a
            // unit toggled off in the UI keeps running server-side.
            for (const tab of accordion.querySelectorAll('.input-accordion:not([data-cnet-wired])')) {
                try {
                    new ControlNetUnitTab(tab, accordion);
                    tab.dataset.cnetWired = '1';
                } catch (e) {
                    // not fully mounted yet — retry on a later UI update
                }
            }
            ...
            // open unit 0 for edit. (Query tabs live — they mount lazily.)
            ...
            const observerAccordionOpen = new MutationObserver(function (mutations) {
                for (const mutation of mutations) {
                    if (!mutation.target.classList.contains('open')) continue;
                    const unitTabs = [...accordion.querySelectorAll('.input-accordion')];
                    if (!unitTabs.length) continue;
                    const anyActive = unitTabs.some(t => {
                        const cb = t.querySelector('.cnet-unit-enabled input') ||
                            gradioApp().querySelector('#' + t.id + '-checkbox input');
                        return (cb && cb.checked) || t.querySelector('.label-wrap').classList.contains('open');
                    });
                    if (!anyActive) unitTabs[0].querySelector('.label-wrap').click();
                }
            });
