...
class ControlNetUiGroup(object):
    ...
    def render(self, tabname: str, elem_id_tabname: str) -> None:
        ...
        with FormRow(elem_classes=["controlnet_main_options"]):
            if getattr(self, "enabled_component", None) is not None:
                # the unit's InputAccordion header checkbox (always mounted, even
                # while the unit is collapsed — see scripts/controlnet.py)
                self.enabled = self.enabled_component
            else:
        ...
        with gr.Row(elem_classes=["controlnet_weight_steps", "controlnet_row"]):
            ...
            # gradio_rangeslider (the old dual-thumb Timestep Range) has no gradio-6
            # build; use two paired gr.Sliders that ARE guidance_start / guidance_end.
            with gr.Row(elem_classes="controlnet_control_step_slider"):
                ...
                self.guidance_end = gr.Slider(
                    ...
                    elem_id=f"{elem_id_tabname}_{tabname}_controlnet_end_control_step_slider",
                    elem_classes="controlnet_end_control_step_slider",
                )
            # self.timestep_range retained as an attribute (some code may reference it)
            # but is no longer a component — the two sliders hold the values directly.
            self.timestep_range = None
        ...
        (
            ...
            outputs=unit,
            # NOT queue=False: gradio 6 sends unqueued events as separate HTTP
            # requests with no ordering guarantee against the queued generate —
            # when masks are cached, generation reached ControlNet before this
            # refresh landed and ran with the units' previous state. Queued
            # listeners on the same trigger run in registration order, and this
            # is registered before the generate handler, so the unit state is
            # always refreshed from the live UI before generation reads it.
        )
    ...
    def register_sync_batch_dir(self):
        ...
        _out_dir = ControlNetUiGroup.a1111_context.img2img_batch_output_dir
        if _out_dir is not None:  # not captured in this Forge UI (e.g. with Replacer loaded) — skip the batch-dir sync
            _out_dir.blur(
                fn=lambda a: a,
                inputs=[_out_dir],
