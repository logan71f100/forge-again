                ...
                with gr.Row(elem_id=elem_id_tabname + "_accordions", elem_classes="accordions"):
                    for i in range(max_models):
                        with InputAccordion(
                            ...
                            elem_classes=["cnet-unit-enabled-accordion", "input-accordion-unlinked"],
                        ) as unit_enabled:
                            group = ControlNetUiGroup(is_img2img, default_unit, photopea)
                            # gradio 6 unmounts a closed accordion's contents, so an
                            # Enable checkbox inside the unit body is unreachable (and
                            # unsyncable) while the unit is collapsed — its frontend
                            # value would stay frozen at whatever last set it. The
                            # accordion header checkbox is always mounted, so use it
                            # as the unit's enabled input directly.
                            group.enabled_component = unit_enabled
        ...
        if unit.input_mode == external_code.InputMode.MERGE:
            ...
            if unit.use_preview_as_input and unit.generated_image is not None:
                ...
                if a1111_i2i_image is None:
                    # txt2img with an enabled unit and no image anywhere -- fail with a
                    # clear message instead of a deep 'NoneType' has no attribute crash
                    raise ValueError(f"ControlNet unit is enabled but has no input image -- upload one in the unit, or disable the unit")
                # txt2img processing has no resize_mode attribute; fall back to the unit's
                resize_mode = external_code.resize_mode_from_value(getattr(p, 'resize_mode', unit.resize_mode))
            ...
            if not isinstance(image, np.ndarray) or image.ndim == 0 or image.size == 0:
