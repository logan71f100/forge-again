...
def webui_worker():
    ...
    while 1:
        ...
        # gradio 6 server-side rendering. OPT-IN (SD_WEBUI_SSR=1) — measured to be
        # a net negative for localhost: it shrinks the initial payload (5.8MB ->
        # ~100KB) which only helps over a slow REMOTE link, and on Forge's slow
        # cold boot gradio's 30s node-verify times out and silently falls back to
        # client render anyway, just wasting ~30s. Only worth it for LAN/remote
        # access. Node is bootstrapped by prepare_node() when this is set.
        _bundled_node = os.path.join(os.path.dirname(__file__), "node",
                                     "node.exe" if os.name == "nt" else "bin/node")
        _ssr_enabled = os.getenv("SD_WEBUI_SSR") == "1" and os.path.isfile(_bundled_node)
        if _ssr_enabled:
            os.environ.setdefault("GRADIO_NODE_PATH", _bundled_node)
